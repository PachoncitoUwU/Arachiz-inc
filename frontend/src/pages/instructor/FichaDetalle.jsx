...
import { 
  ...
  Pin, Plus, ListChecks, MapPin, Building, Calendar, Copy, Fingerprint, Link, Eye, EyeOff, Bell, QrCode, Upload, MessageSquare, Ticket
} from 'lucide-react';
...
export default function FichaDetalle() {
  ...
  const loadFicha = async (showSkeleton = true) => {
    try {
      ...
      // Cargar eventos para el instructor
      try {
        const evRes = await fetchApi('/eventos?role=instructor');
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-white dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm overflow-hidden">
            ...
            <button 
              ...
              className="px-3 py-2 text-sm text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center gap-1 font-medium transition-colors border-l border-gray-200 dark:border-zinc-700"
